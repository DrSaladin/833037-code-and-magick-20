...
var renderCloudStroke = function (ctx, x, y, fillcolor) {
  ctx.strokeStyle = fillcolor;
  ctx.strokeRect(x, y, CLOUD_WIDTH + 2, CLOUD_HEIGHT + 2);
};
...
window.renderStatistics = function (ctx, players, times) {
  renderCloud(ctx, CLOUD_X + GAP, CLOUD_Y + GAP, 'rgba(0, 0, 0, 1)');
  renderCloudStroke(ctx, CLOUD_X + GAP - 1, CLOUD_Y + GAP - 1, 'rgba(0, 0, 0, 1)');
  renderCloud(ctx, CLOUD_X, CLOUD_Y, '#fff');
  renderCloudStroke(ctx, CLOUD_X - 1, CLOUD_Y - 1, 'rgba(0, 0, 0, 1)');
  ...
  var colors = ['rgba(255, 0, 0, 1)', 'rgb(1, 20, 134)', 'rgb(50, 0, 134)', 'rgb(100, 80, 334)'];
  ctx.fillStyle = '#000';

  var maxTime = getMaxElement(times);
  players = sortingNames(players);
